...
while(valid) {
	...
	while( (valid_active == false) && (valid_energy == false)) {
		active         = read_channel_nb_intel(chan_GA2LS_Off_active[7], &valid_active);
	...
	if (valid) {
		...
		while ((iteration_cnt < DockConst_max_num_of_iters) && (rho > DockConst_rho_lower_bound)) {	
			...
			// Telling Krnl_Conf_Arbiter, LS8 is done
			...
			write_channel_intel(chan_LS2Conf_genotype[7], tmp_genotype_ls);
			mem_fence(CLK_CHANNEL_MEM_FENCE);
			...
			while( (intra_valid == false) || (inter_valid == false)) {
				if (intra_valid == false) {
					energyIA_LS_rx = read_channel_nb_intel(chan_Intrae2StoreLS_intrae[7], &intra_valid);
				}
				else if (inter_valid == false) {
					energyIE_LS_rx = read_channel_nb_intel(chan_Intere2StoreLS_intere[7], &inter_valid);
				}
			}
