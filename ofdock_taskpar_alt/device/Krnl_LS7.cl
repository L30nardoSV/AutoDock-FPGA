...
while(valid) {
	...
	while( (valid_active == false) && (valid_energy == false)) {
		active         = read_channel_nb_intel(chan_GA2LS_Off_active[6], &valid_active);
	...
	if (valid) {
		...
		while ((iteration_cnt < DockConst_max_num_of_iters) && (rho > DockConst_rho_lower_bound)) {	
			...
			#if defined (FIXED_POINT_LS7)
			write_channel_intel(chan_LS2Arbiter_end[6], (fixpt_rho < DockConst_rho_lower_bound)?true:false);
			#else
			...
			for (uchar i=0; i<DockConst_num_of_genes; i++) {
				...
				write_channel_intel(chan_LS2Conf_genotype[6], fixedpt_tofloat(fixpt_tmp3));
				...
				write_channel_intel(chan_LS2Conf_genotype[6], tmp3);
				#endif
			...
			while( (intra_valid == false) || (inter_valid == false)) {
				if (intra_valid == false) {
					energyIA_LS_rx = read_channel_nb_intel(chan_Intrae2StoreLS_intrae[6], &intra_valid);
				}
				else if (inter_valid == false) {
					energyIE_LS_rx = read_channel_nb_intel(chan_Intere2StoreLS_intere[6], &inter_valid);
				}
			}
