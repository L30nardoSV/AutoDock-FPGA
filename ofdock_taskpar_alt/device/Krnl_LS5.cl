...
while(valid) {
	...
	while( (valid_active == false) && (valid_energy == false)) {
		active         = read_channel_nb_intel(chan_GA2LS_Off5_active, &valid_active);
		/*
		current_energy = read_channel_nb_intel(chan_GA2LS_LS5_energy,  &valid_energy);
		*/
		current_energy = read_channel_nb_intel(chan_GA2LS_energy[4],  &valid_energy);
	}
	...
	if (valid) {
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			#if defined (FIXED_POINT_LS5)
			/*
			float tmp_gene = read_channel_intel(chan_GA2LS_LS5_genotype);
			*/
			float tmp_gene = read_channel_intel(chan_GA2LS_genotype[4]);
			genotype [i] = fixedpt_fromfloat(tmp_gene);
			#else
			/*
			genotype [i] = read_channel_intel(chan_GA2LS_LS5_genotype);
			*/
			genotype [i] = read_channel_intel(chan_GA2LS_genotype[4]);
			#endif
		}
		...
		while ((iteration_cnt < DockConst_max_num_of_iters) && (rho > DockConst_rho_lower_bound)) {	
			...
			#if defined (FIXED_POINT_LS5)
			write_channel_intel(chan_LS2Arbiter_LS5_end, (fixpt_rho < DockConst_rho_lower_bound)?true:false);
			#else
			write_channel_intel(chan_LS2Arbiter_LS5_end, (rho < DockConst_rho_lower_bound)?true:false);
			#endif
			...
			for (uchar i=0; i<DockConst_num_of_genes; i++) {
				...
				float tmp_prng = read_channel_intel(chan_PRNG2GA_LS5_float_prng);
				mem_fence(CLK_CHANNEL_MEM_FENCE);
				...
				write_channel_intel(chan_LS2Conf_LS5_genotype, fixedpt_tofloat(fixpt_tmp3));
				...
				write_channel_intel(chan_LS2Conf_LS5_genotype, tmp3);
				#endif
			...
			while( (intra_valid == false) || (inter_valid == false)) {
				if (intra_valid == false) {
					energyIA_LS_rx = read_channel_nb_intel(chan_Intrae2StoreLS_LS5_intrae, &intra_valid);
				}
				else if (inter_valid == false) {
					energyIE_LS_rx = read_channel_nb_intel(chan_Intere2StoreLS_LS5_intere, &inter_valid);
				}
			}
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			...
			#if defined (FIXED_POINT_LS5)
			/*
			write_channel_intel(chan_LS2GA_LS5_genotype, fixedpt_tofloat(genotype [i]));
			*/
			write_channel_intel(chan_LS2GA_genotype[4], fixedpt_tofloat(genotype [i]));
			#else
			/*
			write_channel_intel(chan_LS2GA_LS5_genotype, genotype [i]);
			*/
			write_channel_intel(chan_LS2GA_genotype[4], genotype [i]);
			#endif
		}
