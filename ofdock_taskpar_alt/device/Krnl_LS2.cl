...
while(valid) {
	...
	while( (valid_active == false) && (valid_energy == false)) {
		...
		/*
		current_energy = read_channel_nb_intel(chan_GA2LS_LS2_energy,  &valid_energy);
		*/
	...
	if (valid) {
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			#if defined (FIXED_POINT_LS2)
			/*
			float tmp_gene = read_channel_intel(chan_GA2LS_LS2_genotype);
			*/
			float tmp_gene = read_channel_intel(chan_GA2LS_genotype[1]);
			genotype [i] = fixedpt_fromfloat(tmp_gene);
			#else
			/*
			genotype [i] = read_channel_intel(chan_GA2LS_LS2_genotype);
			*/
			genotype [i] = read_channel_intel(chan_GA2LS_genotype[1]);
			#endif
		}
		...
		while ((iteration_cnt < DockConst_max_num_of_iters) && (rho > DockConst_rho_lower_bound)) {	
			...
			for (uchar i=0; i<DockConst_num_of_genes; i++) {
				...
				float tmp_prng = read_channel_intel(chan_PRNG2GA_LS2_float_prng);
				mem_fence(CLK_CHANNEL_MEM_FENCE);
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			...
			#if defined (FIXED_POINT_LS2)
			/*
			write_channel_intel(chan_LS2GA_LS2_genotype, fixedpt_tofloat(genotype [i]));
			*/
			write_channel_intel(chan_LS2GA_genotype[1], fixedpt_tofloat(genotype [i]));
			#else
			/*
			write_channel_intel(chan_LS2GA_LS2_genotype, genotype [i]);
			*/
			write_channel_intel(chan_LS2GA_genotype[1], genotype [i]);
			#endif
		}
